...
class TorcsEnv(gym.Env):
    ...
    def __init__(self, gui=False, infinite=False, port=3001, max_steps=4000):
        ...
        # Analyse observation: angle + 19 track + trackPos + speedX + speedY = 23
    ...
    def _state_from_ob(self, ob):
        return np.hstack((ob.angle, ob.track, ob.trackPos, ob.speedX, ob.speedY)).astype(np.float32)
    ...
    def _reward(self, ob, steer, pedal, prev_steer, prev_pedal):
        ...
        reward -= 0.08 * pedal_change
        # Extra penalty for large steering at higher speed (helps reduce spins).
        reward -= 0.12 * max(0.0, speed_x) * abs(float(steer))
        reward -= 0.18 * max(0.0, speed_x) * steer_change
